...
Extract the gzipped tar archive, run 'make' to compile and, for example, './conf-infomap 344 flow.net 10 100 0.90' to run the code.
...
The code also outputs a file that ends with _sigscores.txt. The second column is 1 if the node is in the significant core and 0 otherwise. The significance score in the third column gives the relative co-clustering with significant core nodes over all bootstrap clusterings, and represents a type of p-value.