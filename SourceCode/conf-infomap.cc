...
void printSignificantTree(string s,multimap<double,treeNode,greater<double> >::iterator it_tM,ofstream *outfile,vector<pair<bool,double> > &significantVec);
void findConfCore(multimap<double,treeNode,greater<double> > &treeMap,vector<vector<int > > &bootClusters,vector<pair<bool,double> > &significantVec,double conf,MTRand *R);
void findConfModules(multimap<double,treeNode,greater<double> > &treeMap,vector<vector<int > > &bootClusters,vector<pair<bool,double> > &significantVec,vector<pair<int,int> > &mergers,double conf);
...
int main(int argc,char *argv[]){
  ...
  for(int bootstrap = 0;bootstrap < Nbootstraps ; bootstrap++){
    ...
    double uncompressedCodeLength = -greedy->nodeSize_log_nodeSize;
  ...
  vector<pair<bool,double> > significantVec = vector<pair<bool,double> >(Nnode);
  ...
  oss.str("");
  oss << networkName << "_sigscores.txt";
  outfile.open(oss.str().c_str());
  outfile << "# node_nr in_sig_core sig_score" << endl;
  for(int i=0;i<Nnode;i++)
    outfile << i+1 << " " << significantVec[i].first << " " << significantVec[i].second << endl;
  outfile.close();
...
void printSignificantTree(string s,multimap<double,treeNode,greater<double> >::iterator it_tM,ofstream *outfile,vector<pair<bool,double> > &significantVec){
  ...
  if(it_tM->second.nextLevel.size() > 0){
    ...
    int i = 1;
    for(multimap<double,pair<int,string>,greater<double> >::iterator mem = it_tM->second.members.begin(); mem != it_tM->second.members.end(); mem++){
      string cpy_s(s + (significantVec[mem->second.first].first == true ? (":") : (";") ) + to_string(i) + " \"" + mem->second.second + "\" " + to_string(mem->first));
      (*outfile) << cpy_s << endl;
      i++;
    }  
  }
  ...
  for(int i=0;i<M;i++){
    ...
    if(N != 1){
      ...
      for(int j=0;j<N;j++){
        double p = 0.0;
        for(int k=0;k<Nboots;k++){
          int modNr = bootClusters[k][modSortMembers[i][j]]; // Node j in cluster i belongs to cluster modNr in resampled network k
